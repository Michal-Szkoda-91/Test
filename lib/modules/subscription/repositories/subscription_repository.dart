...
class SubscriptionRepository implements OnStartService {
  ...
  Future<Subscription> get(String userId) async {
    ...
    if (subscription.isActive) {
      ...
      return subscription.maybeMap(
        orElse: () => subscription,
        active: (active) => active.copyWith(activeOffer: product),
      );
    }
    return subscription;
  ...
  Future<List<SubscriptionProduct>> getOffers() async {
    return _inAppSubscriptionApi.getOffers();
  }
  ...
  Future<void> purchase(SubscriptionProduct product) {
    if (product is RevenueCatProduct) {
      return _inAppSubscriptionApi.purchasePackage(product.revenueCatPackage);
    }
  ...
  Future<void> unsubscribe() async {
    await _inAppSubscriptionApi.disconnectUser();
  }
