...
// ---
// We wrap the Glassfy API to be able to fake it properly (Glassfy use static methods which are hard for tests)
class RevenueCatPaymentApi {
  ...
  Future<void> initUser(String userId) async {
    if (!_hasInit) {
      throw Exception('Glassfy is not initialized');
    }
  ...
  Future<List<SubscriptionProduct>> getOffers() async {
    ...
    return SubscriptionProductAdapter(offerings: offers).transform();
  }
...
class SubscriptionProductAdapter {
  ...
  List<SubscriptionProduct> transform() {
    try {
      ...
      for (final package in offer.availablePackages) {
        products.add(RevenueCatProduct(
          revenueCatOffer: offer,
