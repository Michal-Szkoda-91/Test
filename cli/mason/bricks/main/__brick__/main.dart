...
{{#slang}}import 'package:apparence_kit/i18n/translations.g.dart';{{/slang}}
...
{{#slang}}import 'package:flutter_localizations/flutter_localizations.dart';{{/slang}}
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:go_router/go_router.dart';
import 'package:logger/logger.dart';
{{#sentry}}import 'package:sentry_flutter/sentry_flutter.dart';{{/sentry}}
import 'package:shared_preferences/shared_preferences.dart';
{{#supabase}}import 'package:supabase_flutter/supabase_flutter.dart';{{/supabase}}
...
void main() async {
  ...
  {{#supabase}}// initialize supabase app
  await env.map(
  ...
  {{#sentry}}// initialize sentry for error reporting in production only
  await env.map(
    dev: (_) async => run(sharedPrefs),
    prod: (vars) => SentryFlutter.init(
      (options) {
        options.dsn = vars.sentryDsn;
        options.tracesSampleRate = 0.2; // 20% of traces will be sent to Sentry server. You should start with 1 and decrease it once you have more users. 
  ...
  {{/sentry}}
  {{^sentry}}run(sharedPrefs);{{/sentry}}
}

{{#slang}}void run(SharedPreferences prefs) => runApp(
      TranslationProvider(
        child: ProviderScope(
          child: MyApp(
    ...
    return ThemeProvider(
      ...
      child: Builder(builder: (context) {
        return MaterialApp.router(
          ...
          routerConfig: routes,
          {{#slang}}localizationsDelegates: const [
            GlobalMaterialLocalizations.delegate,
            GlobalWidgetsLocalizations.delegate,
            GlobalCupertinoLocalizations.delegate,
          ],
          locale: TranslationProvider.of(context).flutterLocale,
          supportedLocales: AppLocaleUtils.supportedLocales,{{/slang}}
          ...
          builder: (context, child) => Initializer(
            services: [
              ...
              {{#analytics}}// analytics
              analyticsApiProvider,{{/analytics}}
            ],
            onReady: child!,
